...
const AgendaPage = () => {
  ...
  const [agendaData, setAgendaData] = useState({ classes: [], monitorSessions: [], reminders: [] });
  ...
  const fetchAgenda = useCallback(async () => {
    ...
    try {
      ...
      const data = await agendaService.getDay(formatDate(selectedDate));
      setAgendaData(data);
    } catch {
      setAgendaData({ classes: [], monitorSessions: [], reminders: [] });
    } finally {
      setLoading(false);
    }
  ...
  const classesOnDay = agendaData.classes
    .map(cls => {
      const parsed = classOnDay(cls.schedule, selectedDay);
  ...
  }, [sessionTopic, sessionDate, sessionStart, sessionEnd, sessionLocation, sessionLink, pushNotify]);

  const hasFreeSlot = classesOnDay.length === 0 && sessionsOnDay.length === 0 && remindersOnDay.length === 0;
  ...
  return (
    <div className="w-full min-h-screen flex flex-col bg-gray-50">
          ...
          {weekDays.map((day) => {
            ...
            return (
              ...
              >
                ...
                {isSelected && !hasFreeSlot && <span className="w-1.5 h-1.5 rounded-full bg-white mt-1" />}
                {isSelected && hasFreeSlot && <span className="w-1.5 h-1.5 rounded-full bg-green-400 mt-1" />}
                {isToday && !isSelected && (
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className={`text-xs font-bold px-3 py-1 rounded-full ${
              hasFreeSlot ? 'bg-green-100 text-green-700' : 'bg-gradua-agenda text-white'
            }`}>
              {loading
                ? 'Carregando...'
                : hasFreeSlot
                    ? 'DIA LIVRE'
                    : `${allItems.length} ${allItems.length === 1 ? 'ITEM' : 'ITENS'} HOJE`}
            </span>
          </div>
        </div>

        {/* Botão de agendar sessão — visível apenas para perfil Monitor em dias livres */}
        {isMonitor(user) && hasFreeSlot && (
          <button
      ...
      <div className="flex-1 px-4 py-4 space-y-4 overflow-y-auto pb-28" style={{ maxHeight: 'calc(100vh - 280px)' }}>
        ...
        {loading && (
          <div className="flex items-center justify-center py-12">
            <Loader2 size={28} className="animate-spin text-gradua-agenda" />
          </div>
        )}
        ...
        {!loading && allItems.map((item) => (
          <div key={item.id} className="rounded-2xl overflow-hidden flex-shrink-0 relative group bg-white shadow-[0_2px_8px_rgba(30,58,95,0.08)]">
            ...
            {/* Link de reunião para monitorias */}
            {item.meetingLink && (
      ...
      {/* Modal: agendar sessão de monitoria (apenas Monitor) */}
