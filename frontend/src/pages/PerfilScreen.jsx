...
import { TrendingUp, ChevronRight, FileText, History, KeyRound, LogOut, CheckCircle2, ShieldAlert, Loader2 } from 'lucide-react';
  ...
  <div className="bg-white rounded-3xl p-6 mb-4 flex flex-col items-center gap-3 shadow-[0_2px_8px_rgba(0,0,0,0.04)] relative">
    <div className="relative w-32 h-40 sm:w-40 sm:h-48 mb-2">
      <div className="w-full h-full rounded-2xl overflow-hidden bg-orange-50 shadow-inner">
        <img src={perfil} alt="Foto de Perfil" className="w-full h-full object-cover" />
      </div>
...
// Progresso agora usa dados da API de stats — não mais hardcoded
const ProgressCard = ({ stats, loading }) => {
  const integral = stats?.integralizationPercent ?? null;
  const hours = stats?.hoursPending ?? null;

  // Estima créditos a partir do percentual (base: 240 créditos totais do curso)
...
const PerfilScreen = () => {
  ...
  useEffect(() => {
    let mounted = true;
    // Busca profile e stats em paralelo para ter todos os dados de uma vez
    Promise.all([
