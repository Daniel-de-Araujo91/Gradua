...
public interface ForumVoteRepository extends JpaRepository<ForumVoteModel, UUID> {
    ...
    Optional<ForumVoteModel> findByCommentAndAuthor(ForumCommentModel comment, UserModel author);

    long countByTopicAndVoteType(ForumTopicModel topic, ForumVoteModel.VoteType voteType);
    long countByCommentAndVoteType(ForumCommentModel comment, ForumVoteModel.VoteType voteType);
}
